...
const Button = ({ children, type, onClick }) => {
  ...
  else if (type === "primary-diffuse") {
    return (
      <button
        onClick={onClick}
        className="mob:text-sm laptop:text-base p-2 mob:p-1 laptop:p-2 m-2 mob:m-1 laptop:m-2 rounded-lg text-white bg-gray-400 hover:bg-gray-500 first:ml-0">
        {children}
      </button>
    );
  }
